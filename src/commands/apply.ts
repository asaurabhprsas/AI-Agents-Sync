...
export async function applyCommand(agents: string[]) {
  ...
  const basePersona = fs.existsSync(basePersonaPath) ? fs.readFileSync(basePersonaPath, 'utf-8') : '';
  ...
  const processTarget = (targetDef: any, targetPath: string) => {
    for (const [agentName, agentConfig] of Object.entries(targetDef)) {
      ...
      const rulesConfig = agentConfig as { rules: string[], mcpServers: string[] };
      let rulesContent = '';
      ...
      for (const srv of rulesConfig.mcpServers) {
        if (allMcps[srv]) {
          ...
          console.warn(chalk.yellow(`Warning: MCP server ${srv} not found in mcp.json.`));
        }
      }
